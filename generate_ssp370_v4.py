...
@hydra.main(version_base=None, config_path="./configs", config_name="generate_aero_ssp370_v4")
def main(config: DictConfig) -> None:
    ...
    pg_kwargs = InitProcessGroupKwargs(timeout=datetime.timedelta(seconds=3600))
    accelerator = Accelerator(**config.accelerator, kwargs_handlers=[pg_kwargs])
    ...
    model.ema_model.load_state_dict(ema_state)
    model.eval()

    with torch.no_grad():
        ema_p = next(model.ema_model.parameters())
        raw_p = next(model.model.parameters())

    print("[EMA CHECK][generation]")
    print("  EMA mean abs :", ema_p.abs().mean().item())
    print("  RAW mean abs :", raw_p.abs().mean().item())
    print("  EMA == RAW? :", torch.allclose(ema_p, raw_p))
    ...
    if getattr(dataset, "normalizer", None) is not None:
        xr_ds = xr_ds.map(lambda da: dataset.normalizer.normalize(da, varname=da.name))
    ...
    for i in tqdm(range(config.samples_per)):
        ...
        if accelerator.is_main_process:
            ...
            #     "saliency_co2": gen_sal_co2,           # attribution for CO₂
