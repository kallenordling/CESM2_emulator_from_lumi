...
# `PRECT_model` with dims (member, year, lat, lon), in mm/day. The global mean
...
for key in SCENARIOS:
    ...
    #
    # eval_aero writes `PRECT_model` with dims (member, year, lat, lon), so the
    # whole ensemble arrives as one array and nothing has to be reassembled.
    field = ds["PRECT_model"]
...
for key, (label, _, colour) in SCENARIOS.items():
    ...
    stats[key] = dict(
        ...
        # mostly the shared forced trend, so read it next to the bias rather
        # than on its own.
        corr=float(np.corrcoef(emulator_mean, cesm_mean)[0, 1]),
        inside=float(((difference >= spread_low)
                      & (difference <= spread_high)).mean()) * 100)
    print(f"[step 7] {key:7s} r {stats[key]['corr']:.3f}, "
          f"bias {stats[key]['bias']:+.3f} %-points, "
...
# =============================================================================
#  STEP 12 — the same numbers as a LaTeX table
# =============================================================================
# Written as a bare `tabular` so it can be \\input inside whatever table
# environment the paper wants, with its own caption and placement. booktabs
# rules; siunitx is not assumed.
...
for key in SCENARIOS:
    ...
    row = stats[key]
    rows_tex.append(
        f"{SCENARIOS[key][0]} & {row['n_emu']} & {row['n_cesm']} & "
...
with open(TABLE, "w") as fh:
    fh.write("% Precipitation: emulated vs held-out CESM2 global means.\n"
             ...
             "% Generated by scripts/make_fig2.py — do not edit by hand.\n")
    fh.write(table_tex + "\n")
print(f"[step 12] wrote {TABLE}")
