...
def standardize(ds: xr.DataArray) -> xr.DataArray:
    ds_emis = xr.open_dataset(EMISSIONS_PATH)[ds.name]  # <-- valitse muuttuja

    mean = ds_emis.mean()
    std = ds_emis.std()

    # std on nyt DataArray (0D tai broadcastoitava)
    if float(std.values) == 0.0:
...
class ClimateDataset(Dataset):
    ...
    def load_data(self, realization: str):
        ...
        self.dataset_cond = self.dataset_cond.map(standardize)
