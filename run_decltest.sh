...
#
# ISOLATED A/B test of the declining-aerosol additivity prototype (option 1).
# Forks the newest production run_slope-tcre checkpoint into a SEPARATE save_name
# (run_decltest) with interaction_decline_frac=0.2 enabled — so it does NOT touch
# the production chain. No eval watcher, no self-chain (single job). After it
# runs, eval a run_decltest checkpoint:
#     CHECKPOINT=.../runs/run_decltest_<ep>.pt sbatch run_eval_aero.sh
# Success = ssp126 ratio drops vs production at matched epochs while hist/ssp370
# hold near 1.0. NOTE: reads cond/data directly from /scratch (no /tmp staging),
# so epochs are a bit slower — fine for a test.
...
echo "[fork] forking declining-aerosol test from ${PROD_CKPT}"
...
RUN_CMD="singularity exec ${SIF} bash -c '
    accelerate launch \
        ...
        main_aero.py \
        trainer.hyperparameters.save_name=run_decltest.pt \
        trainer.hyperparameters.load_path=${PROD_CKPT} \
        trainer.hyperparameters.interaction_decline_frac=0.2
'"
