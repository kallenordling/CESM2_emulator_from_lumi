#!/bin/bash
#SBATCH --job-name=diffusion_lrdecayoff
#
# ── LR-decay A/B arm (explicit constant-LR baseline, fork of run2_mseyb.sh) ────────────────────
# A/B for the LR-decay-vs-constant-LR "wander" fix. This arm holds the constant base LR (lr_decay=off); it is the
# matched baseline for run2_lrdecay.sh (cosine). Both seed the same epoch and
# share all other settings, so any divergence is attributable to the LR
# schedule alone (data_config=config_data_ybias.yaml, mse_only=true → all aux
# losses OFF for max wander).
#   save_name = run_lrdecayoff (own checkpoints/evals)
#
# SEED THE NAMESPACE FIRST (save_name-scoped "newest" then resumes at that epoch):
#   cp runs/run_mseyb_640.pt runs/run_lrdecayoff_640.pt
# (use a recent mseyb checkpoint on LUMI; the matched off-arm seeds the same epoch).
# Fire:  CHAIN_REMAINING=4 sbatch run2_lrdecayoff.sh
...
#   CHAIN_REMAINING=4 sbatch run2_lrdecayoff.sh
...
if [[ "${CHAIN_REMAINING}" -gt 1 ]]; then
    NEXT_JOB=$(sbatch --parsable \
           ...
           --chdir="${SLURM_SUBMIT_DIR}" \
           "${SLURM_SUBMIT_DIR}/run2_lrdecayoff.sh" 2>/dev/null) || NEXT_JOB=""
...
RUN_CMD="singularity exec --bind ${LOCAL_DATA_ROOT}:${SRC_DATA_ROOT} ${SIF} bash -c '
    accelerate launch \
        ...
        trainer.hyperparameters.save_name=run_lrdecayoff.pt \
        trainer.hyperparameters.mse_only=true \
        trainer.hyperparameters.lr_decay=off
'"
