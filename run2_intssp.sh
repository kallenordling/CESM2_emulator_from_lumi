#!/bin/bash
#SBATCH --job-name=diffusion_intssp
...
#
# ── interaction-match-ssp370 A/B arm (isolated fork of run2_aero.sh) ─────────
# = run_sensfix config (B+C, SUL clip) PLUS interaction_match_ssp370=true:
# the interaction loss also pins ssp370's global-mean interaction to CESM2's
# measured sub-additive value (not →0), to build a compositional operator that
# generalizes to the unseen ssp126. Compare run_intssp vs run_sensfix (esp.
# ssp126 bias/TCRE-ratio) via plot_training_dashboard.py.
#   save_name = run_intssp (own checkpoints/evals, from epoch 0)
# Submit:  CHAIN_REMAINING=12 sbatch run2_intssp.sh
...
if [[ -n "${EXISTING_WATCHER}" ]]; then
    ...
    WATCHER_JOB=$(sbatch --job-name=eval_watcher \
           ...
           --export="ALL,PROD_RUN=run_sensfix,run_intssp" \
...
if [[ "${CHAIN_REMAINING}" -gt 1 ]]; then
    NEXT_JOB=$(sbatch --parsable \
           ...
           --chdir="${SLURM_SUBMIT_DIR}" \
           "${SLURM_SUBMIT_DIR}/run2_intssp.sh" 2>/dev/null) || NEXT_JOB=""
...
RUN_CMD="singularity exec --bind ${LOCAL_DATA_ROOT}:${SRC_DATA_ROOT} ${SIF} bash -c '
    accelerate launch \
        ...
        main_aero.py \
        trainer.hyperparameters.save_name=run_intssp.pt \
