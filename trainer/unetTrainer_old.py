...
def calc_mse_loss2(model_output, target, lats, polar_boost=3.0, polar_lat=60.0):
    ...
    # 1) area weight (cos(lat)) but normalize to mean=1 so loss scale stays stable
    w = torch.cos(lat_rad).clamp_min(0.0)
    w = w / w.mean()

    # 2) optional extra emphasis on polar caps
    polar = (latitude.abs() >= polar_lat).to(spatial_loss.dtype)
    w = w * (1.0 + (polar_boost - 1.0) * polar)   # e.g. 3x weight poleward of 60°

    # broadcast over (B,C,T,H,W) with H=lat dim
    return torch.einsum('...yx,y->...yx', spatial_loss, w).mean()
...
def wrap_lon(x: torch.Tensor, pad: int) -> torch.Tensor:
    # x: (B,C,T,H,W) or anything with lon as last dim
    if pad <= 0:
        return x
    return torch.cat([x[..., -pad:], x, x[..., :pad]], dim=-1)

def crop_lon(x: torch.Tensor, pad: int) -> torch.Tensor:
    if pad <= 0:
        return x
    return x[..., pad:-pad]
...
class UNetTrainer:
    ...
    def get_loss(self, batch,cond_map):
        ...
        with self.accelerator.accumulate(self.model):
            #lon_pad = 8  # try 8 or 16; 8 is usually enough

            #noisy_w = wrap_lon(noisy_samples, lon_pad)
            #cond_w  = wrap_lon(cond_map, lon_pad)

            #model_output_w = self.model(
            #    noisy_w,
            #    timesteps,
            #    cond_map=cond_w,
            #)

            #model_output = crop_lon(model_output_w, lon_pad)
            ...
            #mse_loss = calc_mse_loss2(model_output, target, self.train_set.lats, polar_boost=1.2)
            ...
            # Calculate the loss
            #tv = tv_loss(pred_original_sample)

            loss = mse_loss + cond_loss * self.cond_loss_scaling # + 1e-4 * tv
    ...
    def load(self, path):
        ...
        denom = self.num_steps_per_epoch * self.accelerator.gradient_accumulation_steps
        if denom <= 0:
            self.resume_step = 0
            self.first_epoch = 0
            if self.accelerator.is_main_process:
                self.accelerator.print(
                    f"[WARN] denom for resume_step is {denom}. Forcing resume_step=0. "
                    f"(num_steps_per_epoch={self.num_steps_per_epoch}, grad_accum={self.accelerator.gradient_accumulation_steps})"
               )
        else:
            self.resume_step = self.resume_global_step % denom
